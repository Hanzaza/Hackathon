...
export const EntrepreneurRequestsTab: React.FC<EntrepreneurRequestsTabProps> = ({
  ...
  const handleReject = async (id: string) => {
    setProcessingId(id);
    try {
      const res = await adminService.rejectEntrepreneurRequest(id, 'Revisión administrativa: No cumple con los criterios requeridos.');
      setFeedbackMessage({ text: res.message, type: 'success' });
      if (selectedRequest?.id === id) {
    ...
    <div className="space-y-6">
      ...
      {feedbackMessage && (
        ...
        >
          <div className="flex items-center gap-2">
            {feedbackMessage.type === 'success' ? <Check className="w-4 h-4 text-emerald-600" /> : <AlertCircle className="w-4 h-4 text-rose-600" />}
      ...
      <div className="grid grid-cols-1 gap-4">
        {filteredRequests.length === 0 ? (
          <div className="p-12 text-center rounded-3xl border border-slate-200 bg-white text-slate-500 shadow-xs">
            ...
            <p className="font-bold text-sm text-slate-800">No hay solicitudes en esta sección</p>
            <p className="text-xs mt-1 text-slate-500">No se encontraron registros con los filtros seleccionados.</p>
          </div>
        ) : (
          filteredRequests.map((req) => (
            <div
              key={req.id}
              className="p-5 rounded-3xl border border-slate-200 bg-white hover:shadow-md transition-all flex flex-col lg:flex-row lg:items-center justify-between gap-5 shadow-xs"
            >
              {/* User and Business info */}
              <div className="flex items-start gap-4">
                <div className="relative w-12 h-12 rounded-2xl overflow-hidden bg-purple-50 border border-purple-200 shrink-0">
                  <Image
                ...
                <div className="space-y-1">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="text-base font-black text-slate-950">{req.business_name}</h3>
                    ...
                    {req.status === 'pending' && (
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black bg-amber-100 text-amber-900 border border-amber-300 flex items-center gap-1">
                        <Clock className="w-3 h-3 text-amber-700" /> Pendiente
                      </span>
                    )}
                    {req.status === 'approved' && (
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black bg-emerald-100 text-emerald-900 border border-emerald-300 flex items-center gap-1">
                        <CheckCircle className="w-3 h-3 text-emerald-700" /> Aprobada
                      </span>
                    )}
                    {req.status === 'rejected' && (
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black bg-rose-100 text-rose-900 border border-rose-300 flex items-center gap-1">
                        <XCircle className="w-3 h-3 text-rose-700" /> Rechazada
                      </span>
                    )}
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500">
                    <span className="flex items-center gap-1 text-slate-700 font-medium">
                      <User className="w-3.5 h-3.5 text-purple-600" /> {req.user_name}
                    </span>
                    <span>•</span>
                    ...
                    {req.city && (
                      <>
                        <span>•</span>
                        <span className="flex items-center gap-1">
                          <MapPin className="w-3.5 h-3.5 text-emerald-600" /> {req.city}
                        </span>
                      </>
                    )}
                  </div>
              ...
              <div className="flex flex-wrap items-center gap-2 self-end lg:self-center shrink-0">
                <button
                  ...
                  className="px-3.5 py-2 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-slate-900 text-xs font-bold border border-slate-200 transition-all flex items-center gap-1.5 cursor-pointer"
                >
                ...
                {req.status === 'pending' && (
                  <>
                    <button
                      ...
                      className="px-4 py-2 rounded-2xl bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-xs font-black transition-all shadow-md shadow-emerald-600/25 flex items-center gap-1.5 cursor-pointer active:scale-95"
                    >
                    ...
                    <button
                      ...
                      className="px-3 py-2 rounded-2xl bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold border border-rose-200 transition-all flex items-center gap-1 cursor-pointer"
                    >
      ...
      {selectedRequest && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-2xl rounded-3xl bg-white border border-slate-200 p-6 sm:p-8 space-y-6 max-h-[90vh] overflow-y-auto text-slate-900 shadow-2xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
                ...
                <p className="text-slate-500">{selectedRequest.user_email}</p>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
                ...
                <p className="text-slate-500">{selectedRequest.address || 'Sin dirección especificada'}</p>
              </div>
            </div>

            <div className="space-y-2">
              <span className="text-xs font-bold text-purple-900">Motivación y Propuesta de Valor:</span>
            ...
            {selectedRequest.documents && selectedRequest.documents.length > 0 && (
              <div className="space-y-2">
                <span className="text-xs font-bold text-purple-900">Documentos y Licencias Adjuntas:</span>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {selectedRequest.documents.map((doc: string, idx: number) => (
                    <div
                      key={idx}
                      className="flex items-center gap-2 p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-700"
                    >
              ...
              {selectedRequest.status === 'pending' && (
                <>
                  <button
                    ...
                    className="px-4 py-2.5 rounded-2xl bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold text-xs border border-rose-200"
                  >
                    Rechazar
                  </button>
                  <button
                    ...
                    className="px-5 py-2.5 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs shadow-md shadow-emerald-600/25 flex items-center gap-2"
                  >
                    <Check className="w-4 h-4" />
