...
export default function MapaInmersivo({ 
  ...
  return (
    <div className="relative w-full h-full min-h-[580px] sm:min-h-[680px] lg:min-h-[820px] rounded-2xl sm:rounded-[2.5rem] overflow-hidden shadow-[0_30px_90px_rgba(0,0,0,0.35)] border border-white/20 bg-slate-950 select-none">
      
      {/* ================= MAP CANVAS ================= */}
      <div ref={mapContainer} className="w-full h-full min-h-[580px] sm:min-h-[680px] lg:min-h-[820px]" />
      ...
      <div className="absolute top-2.5 sm:top-3.5 left-2.5 sm:left-3.5 z-20">
        <button 
          type="button"
          onClick={onBack}
          className="h-9 sm:h-10 px-3 sm:px-4 rounded-full bg-slate-900/90 backdrop-blur-xl text-white font-bold text-xs shadow-lg border border-slate-700/80 hover:scale-105 active:scale-95 transition-all flex items-center gap-1.5 sm:gap-2 cursor-pointer"
          aria-label="Volver"
        >
      ...
      <div className="absolute top-2.5 sm:top-3.5 left-1/2 -translate-x-1/2 z-20 max-w-[170px] xs:max-w-xs sm:max-w-md pointer-events-none">
        <div className="flex items-center gap-1.5 sm:gap-2 px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-full bg-slate-950/85 backdrop-blur-xl border border-white/15 text-white shadow-2xl truncate">
          ...
          <span className="text-[10px] text-purple-300 font-medium hidden sm:inline">
      ...
      <div className="absolute top-2.5 sm:top-3.5 right-2.5 sm:right-3.5 z-20 flex flex-col gap-1.5 sm:gap-2">
        ...
        <button
          type="button"
          onClick={toggle3D}
          className="h-9 w-9 sm:h-10 sm:w-10 rounded-full bg-slate-900/90 backdrop-blur-xl text-white font-black text-[11px] shadow-lg border border-slate-700/80 flex items-center justify-center hover:scale-105 active:scale-95 transition-all cursor-pointer"
          title="Alternar vista 3D/2D"
        >
          <span className={is3D ? 'text-purple-400' : 'text-slate-400'}>
            {is3D ? '3D' : '2D'}
          </span>
        </button>
        ...
        <button
          type="button"
          onClick={recenterMap}
          className="h-9 w-9 sm:h-10 sm:w-10 rounded-full bg-slate-900/90 backdrop-blur-xl text-white shadow-lg border border-slate-700/80 flex items-center justify-center hover:scale-105 active:scale-95 transition-all cursor-pointer"
          title="Recentrar mapa"
        >
          <Navigation className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-400" />
        </button>
        ...
        <div className="relative">
          ...
          >
            <Layers className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-200" />
          </button>

          {/* Menú flotante de capas */}
          {showStyleMenu && (
            <div className="absolute right-0 top-11 sm:top-12 w-36 rounded-2xl bg-slate-900/95 backdrop-blur-2xl p-1.5 shadow-2xl border border-slate-700 flex flex-col gap-1 z-30 animate-fadeIn">
      ...
      <div className="absolute top-13 sm:top-16 left-2.5 sm:left-3.5 right-14 sm:right-3.5 z-10 overflow-x-auto no-scrollbar flex items-center gap-1.5 py-1">
        <button
          ...
          className={`px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full text-[11px] sm:text-xs font-bold whitespace-nowrap shadow-md backdrop-blur-md transition-all active:scale-95 cursor-pointer ${
            activeCategory === 'todos'
              ? 'bg-purple-600 text-white border border-purple-400'
              : 'bg-slate-900/80 text-slate-200 border border-slate-700/50 hover:bg-slate-900'
          }`}
        >
      ...
      >
        <div className="w-full rounded-[2rem] bg-slate-900/95 backdrop-blur-2xl border border-slate-700/70 shadow-[0_-15px_40px_rgba(0,0,0,0.4)] p-3.5 sm:p-5 flex flex-col justify-between text-white">
          ...
          >
            <div className="w-10 h-1 bg-slate-600 rounded-full group-hover:bg-purple-400 transition-colors" />
          </button>
            ...
            <div className="flex items-start gap-3 sm:gap-4 mb-2.5">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5">
                  <span className="px-2 py-0.5 rounded-full bg-purple-950 text-purple-300 border border-purple-700/50 text-[9px] sm:text-[10px] font-extrabold uppercase tracking-wide">
                ...
                <p className="text-[10px] sm:text-xs text-slate-300 line-clamp-2 mt-0.5">
                  {selectedPoint.desc}
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-1.5 sm:gap-2 pt-2 border-t border-slate-800">
            {/* Botón 1: Explorar Punto en Mapa */}
            <button
              type="button"
              onClick={() => focusOnPoint(selectedPointIndex)}
              className="inline-flex items-center justify-center gap-1 py-2 sm:py-2.5 px-2 sm:px-3 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-[11px] sm:text-xs shadow-md active:scale-98 transition-all cursor-pointer"
            >
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-1 py-2 sm:py-2.5 px-2 sm:px-3 rounded-xl bg-slate-800 text-slate-100 hover:bg-slate-700 font-bold text-[11px] sm:text-xs transition-all active:scale-98 cursor-pointer"
            >
            ...
            <Link
              ...
              className="hidden sm:inline-flex items-center justify-center gap-1 py-2 sm:py-2.5 px-2 sm:px-3 rounded-xl bg-white text-slate-900 font-bold text-xs shadow-md transition-all active:scale-98 text-center"
            >
              <span>Guía</span>
          ...
          {sheetExpanded && (
            <div className="mt-4 pt-3 border-t border-slate-200 dark:border-slate-800 max-h-56 overflow-y-auto pr-1 flex flex-col gap-2 animate-fadeIn">
                ...
                <button
                  ...
                  className={`w-full p-2.5 rounded-2xl flex items-center justify-between text-left transition-all cursor-pointer ${
                    selectedPointIndex === idx
                      ? 'bg-purple-50 dark:bg-purple-950/60 border border-purple-400/50'
                      : 'hover:bg-slate-100 dark:hover:bg-slate-800/60 border border-transparent'
                  }`}
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <span className={`h-6 w-6 rounded-full flex items-center justify-center text-xs font-black shrink-0 ${
                      selectedPointIndex === idx ? 'bg-purple-600 text-white' : 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300'
                    }`}>
                      {idx + 1}
                    </span>
                  ...
                  <span className="text-purple-600 dark:text-purple-400 font-bold text-xs shrink-0 ml-2">
              ...
              <div className="pt-2">
                <Link
                  href={`/ciudades-creativas/${cityData.slug}`}
                  className="w-full py-2.5 px-4 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold text-xs flex items-center justify-center gap-2"
                >
