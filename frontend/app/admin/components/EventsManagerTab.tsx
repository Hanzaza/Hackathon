    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {events.map((event) => (
          <div
            key={event.id}
            className="rounded-3xl border border-slate-200 bg-white overflow-hidden shadow-xs transition-all duration-300 hover:border-purple-300 hover:shadow-md flex flex-col justify-between"
          >
            <div>
              <div className="relative h-40 w-full bg-slate-100">
                ...
                <div className="absolute top-3 left-3">
                  <span className="px-3 py-1 rounded-full text-[10px] font-black bg-purple-700/90 backdrop-blur-md text-white">
                    {event.category}
                  </span>
                </div>
              ...
              <div className="p-4 space-y-2">
                ...
                <div className="pt-2 border-t border-slate-100 space-y-1.5 text-xs text-slate-500">
                  <div className="flex items-center gap-1.5 text-slate-700">
                    ...
                    <span className="truncate font-medium">{event.location_name} • {event.city}</span>
                  </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-lg rounded-3xl bg-white border border-slate-200 p-6 sm:p-8 space-y-5 text-slate-900 shadow-2xl">
            
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-purple-700" />
                <h3 className="text-lg font-black text-slate-950">Publicar Evento Cultural</h3>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-700 font-bold mb-1">Título del Evento *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-slate-700 font-bold mb-1">Lugar / Sede</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-700 font-bold mb-1">Fecha y Hora de Inicio</label>
                  <input
                    type="datetime-local"
                ...
                <div>
                  <label className="block text-slate-700 font-bold mb-1">Fecha y Hora de Fin</label>
                  <input
                    type="datetime-local"
