...
export const CitiesManagerTab: React.FC<CitiesManagerTabProps> = ({ cities, onRefresh }) => {
  ...
  const handleToggleCreative = async (city: MunicipalityItem) => {
    setTogglingId(city.id);
    try {
      await adminService.toggleCityCreativeStatus(city.id, !city.is_creative);
      onRefresh();
    } finally {
      setTogglingId(null);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-slate-900/70 p-5 sm:p-7 rounded-3xl border border-white/10 backdrop-blur-2xl shadow-xl shadow-black/20">
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-500/10 text-purple-300 border border-purple-500/20 text-xs font-bold uppercase tracking-wider mb-2">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full bg-slate-950/80 border border-white/10 rounded-2xl pl-10 pr-4 py-2.5 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-purple-500 transition-all"
            />
          </div>
          ...
          <button
            ...
            className="w-full sm:w-auto px-5 py-2.5 rounded-2xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-black text-xs shadow-lg shadow-purple-600/30 flex items-center justify-center gap-2 cursor-pointer transition-all active:scale-95 shrink-0"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {filteredCities.map((city) => {
          ...
          return (
            <div
              key={city.id}
              className={`relative rounded-3xl border transition-all duration-300 backdrop-blur-xl overflow-hidden flex flex-col justify-between ${
                isActive
                  ? 'bg-slate-900/80 border-white/10 shadow-xl shadow-black/20 hover:border-purple-500/40'
                  : 'bg-slate-950/90 border-amber-500/30 ring-1 ring-amber-500/20'
              }`}
            >
              ...
              <div className="relative h-40 w-full overflow-hidden bg-slate-950">
                ...
                {city.logo_url && (
                  <div className="absolute top-3 left-3 w-12 h-12 rounded-2xl bg-slate-900/90 backdrop-blur-md p-1 border border-white/20 shadow-lg overflow-hidden">
                    <Image
                      src={city.logo_url}
                      alt={`Logo ${city.name}`}
                      width={48}
                ...
                <div className="absolute top-3 right-3 flex items-center gap-1.5">
                  ...
                  {city.is_creative && (
                    <span className="px-2 py-1 rounded-full text-[10px] font-black bg-purple-500/30 text-purple-200 border border-purple-400/40 backdrop-blur-md flex items-center gap-1">
                ...
                <div className="absolute bottom-3 left-4 right-4">
                  ...
                  <p className="text-xs text-purple-300 flex items-center gap-1 font-semibold">
              ...
              <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                <div>
                  {city.subtitle && (
                    <p className="text-xs font-bold text-slate-300 italic mb-2">
                      &quot;{city.subtitle}&quot;
                    </p>
                  )}
                  ...
                  {city.specialties && city.specialties.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      {city.specialties.slice(0, 3).map((spec, i) => (
                        <span
                          key={i}
                          className="px-2 py-0.5 rounded-lg bg-white/5 border border-white/10 text-[10px] font-medium text-slate-300"
                        >
                          {spec}
                        </span>
                      ))}
                      {city.specialties.length > 3 && (
                        <span className="px-1.5 py-0.5 rounded-lg bg-white/5 text-[10px] text-slate-500">
                          +{city.specialties.length - 3}
                        </span>
                      )}
                    </div>
                ...
                <div className="p-3 rounded-2xl bg-slate-950/70 border border-white/5 flex items-center justify-between">
                  ...
                  >
                    {isActive ? (
                      <ToggleRight className="w-8 h-8 text-emerald-400 hover:text-emerald-300 transition-transform active:scale-95" />
                    ) : (
                      <ToggleLeft className="w-8 h-8 text-amber-500 hover:text-amber-400 transition-transform active:scale-95" />
                    )}
                  </button>
                </div>
                ...
                <div className="pt-3 border-t border-white/10 flex items-center justify-between gap-2">
                  <Link
                    ...
                    target="_blank"
                    className="px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white text-xs font-bold transition-all flex items-center gap-1.5 border border-white/10"
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className="p-2 rounded-xl bg-purple-600/20 hover:bg-purple-600/30 text-purple-300 hover:text-white border border-purple-500/30 text-xs font-bold transition-all flex items-center gap-1 cursor-pointer"
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/80 backdrop-blur-md animate-fadeIn">
          <div className="relative w-full max-w-2xl max-h-[90vh] rounded-[2.5rem] bg-slate-900 border border-white/15 shadow-2xl p-6 sm:p-8 flex flex-col overflow-hidden text-slate-100">
            ...
            <div className="flex items-center gap-2 my-4 p-1 rounded-2xl bg-slate-950 border border-white/10">
              {[
            ...
            <form onSubmit={handleSaveCity} className="flex-1 overflow-y-auto pr-1 space-y-4 text-xs">
                ...
                <div className="space-y-4 animate-fadeIn">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-slate-950 border border-white/10 rounded-xl px-3 py-2.5 text-white placeholder:text-slate-600 focus:outline-none focus:border-purple-500"
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-slate-950 border border-white/10 rounded-xl p-3 text-white placeholder:text-slate-600 focus:outline-none focus:border-purple-500 leading-relaxed"
                    />
                  </div>
                  ...
                  <div className="p-3.5 rounded-2xl bg-slate-950 border border-white/10 flex items-center justify-between">
                    <div>
                      <span className="text-[11px] font-black text-white block">Estado de Disponibilidad</span>
              ...
              {modalTab === 'media' && (
                <div className="space-y-4 animate-fadeIn">
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 bg-slate-950 border border-white/10 rounded-xl px-3 py-2.5 text-white placeholder:text-slate-600 focus:outline-none focus:border-purple-500"
                      />
                      {formLogoUrl && (
                        <div className="w-10 h-10 rounded-xl bg-slate-950 border border-white/20 overflow-hidden shrink-0">
                  ...
                  <div>
                    ...
                    />
                    {formHeroDesktop && (
                      <div className="relative h-28 w-full rounded-xl overflow-hidden mt-2 border border-white/10">
              ...
              {modalTab === 'geo' && (
                <div className="space-y-4 animate-fadeIn">
                  ...
                  <div className="p-4 rounded-2xl bg-purple-950/40 border border-purple-500/20 text-purple-200 flex items-start gap-3">
                    <Compass className="w-5 h-5 text-purple-400 shrink-0 mt-0.5" />
                    <div>
                      <h4 className="font-bold text-white text-xs">Integración con el Mapa Inmersivo</h4>
              ...
              <div className="pt-4 border-t border-white/10 flex items-center justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-slate-300 font-bold text-xs transition-all cursor-pointer"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-black text-xs shadow-lg shadow-purple-600/30 transition-all flex items-center gap-2 cursor-pointer disabled:opacity-50"
                >
      ...
      {deletingCity && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
          <div className="relative w-full max-w-md rounded-3xl bg-slate-900 border border-rose-500/30 p-6 text-center shadow-2xl">
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-black text-xs shadow-lg shadow-rose-600/30 transition-all cursor-pointer disabled:opacity-50"
              >
